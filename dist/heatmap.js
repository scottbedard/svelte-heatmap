...
function normalizedHistory(colors, emptyColor, history) {
	return normalize({
    colors: colors,
...
function encapsulateStyles(node) {
	setAttribute(node, "svelte-2145756061", "");
}
...
function Heatmap(options) {
	...
	this._recompute({ colors: 1, emptyColor: 1, history: 1 }, this._state);
...
Heatmap.prototype._recompute = function _recompute(changed, state) {
	if (changed.colors || changed.emptyColor || changed.history) {
		if (differs(state.normalizedHistory, (state.normalizedHistory = normalizedHistory(state.colors, state.emptyColor, state.history)))) changed.normalizedHistory = true;
	}
};
