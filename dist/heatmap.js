...
function attachDayColor(normalizedHistory, emptyColor) {
...
function normalizedHistory(emptyColor, history) {
	return normalize({
    history: history || [],
...
function encapsulateStyles(node) {
	setAttribute(node, "svelte-2420224710", "");
}
...
function Heatmap(options) {
	...
	this._recompute({ emptyColor: 1, history: 1 }, this._state);
...
Heatmap.prototype._recompute = function _recompute(changed, state) {
	if (changed.emptyColor || changed.history) {
		if (differs(state.normalizedHistory, (state.normalizedHistory = normalizedHistory(state.emptyColor, state.history)))) changed.normalizedHistory = true;
	}
};
